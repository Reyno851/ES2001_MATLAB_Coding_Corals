...
% 
% This script specifically plots data of SST over the time period of 2000
...
% empty vectors for latitude, longitude, year, and SST are created. These
...
for aa = 1:1:length(filenames)
    ...
    % Concatenate latitude , longitude, year and SST data from each .txt
...
% longitudes, SST and corresponding year of record
data_matrix = [lat_v lon_v year_v sst_v];
...
for yy = min(year_v) : 1 : max(year_v)
    ...
    % the corresponding latitude, longitude and SST data.
    ...
    c.Label.String = 'Sea Surface Temperature (°C)';
    c.Label.FontSize = 12;
    
    % Use scatterm to plot all latitude and longitude of each NOAA Coral Reef Watch Virtual
    % Station and color of each station plot depends on the SST of the
    % station of each year.
    scatterm (lat_v(index), lon_v(index), 50, sst_v(index));
    ...
    title (['Plot of Sea Surface Temperature in North Pacific Stations in ',year_str],'FontSize',25);
    ...
    if yy == min(year_v) 
        imwrite(imind,cm,'npmap_sst.gif','gif', 'Loopcount',inf); 
    
    else
        
        imwrite(imind,cm,'npmap_sst.gif','gif','WriteMode','append');
