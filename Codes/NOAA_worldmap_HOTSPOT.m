...
% 
% This script specifically plots the Maximum Hotspots on a
% world map.
...
for aa = 1:length (hdf_names)
    ...
    max_hotspot = hdfread (myFile, 'CRW_HOTSPOT_MAX');
    ...
    % under "variable_info". 
    % In this case, hotspots do not have large negative values,
    % and raw data needs to be divided by 100 to get data in °C.
    max_hotspot(max_hotspot==-7777)= NaN;
    max_hotspot=max_hotspot *0.01;
    ...
    % each image frame
    title (['Map of Maximum Bleaching Hotspots in Year ',year_str], 'FontSize',25);
    ...
    if aa == 1 
        ...
        imwrite(imind,cm,'hotspot_max.gif','gif', 'Loopcount',inf); 
    
    else
        % WriteMode and Append appends images to first image.
        imwrite(imind,cm,'hotspot_max.gif','gif','WriteMode','append');
