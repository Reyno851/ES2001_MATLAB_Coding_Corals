...
% 
% This script specifically plots the Maximum SST Anomaly on a
% world map.
...
for aa = 1:length (hdf_names)
    ...
    max_sstanom = hdfread (myFile, 'CRW_SSTANOMALY_MAX');
    ...
    max_sstanom(max_sstanom==-7777)= NaN;
    max_sstanom = max_sstanom*0.01;
    ...
    % each image frame
    title (['Map of Maximum Sea Surface Temperature Anomaly in Year ',year_str],'FontSize',25);
    ...
    if aa == 1 
        ...
        imwrite(imind,cm,'sstanom_max.gif','gif', 'Loopcount',inf); 
    
    else
        % WriteMode and Append appends images to first image.
        imwrite(imind,cm,'sstanom_max.gif','gif','WriteMode','append');
