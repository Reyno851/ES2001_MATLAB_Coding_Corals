...
% 
% This script specifically plots data of DHW over the time period of 2000
...
% empty vectors for latitude, longitude, year, and DHW are created. These
...
for aa = 1:1:length(filenames)
    ...
    % Concatenate latitude , longitude, year and DHW data from each .txt
...
% longitudes, DHW and corresponding year of record
data_matrix = [lat_v lon_v year_v dhw_v];
...
for yy = min(year_v) : 1 : max(year_v)
    ...
    % the corresponding latitude, longitude and DHW data.
    ...
    c.Label.String = 'Degree Heating Weeks (°C Weeks)';
    c.Label.FontSize= 12;
    
    % Use scatterm to plot all latitude and longitude of each NOAA Coral Reef Watch Virtual
    % Station and color of each station plot depends on the DHW of the
    % station of each year.
    scatterm (lat_v(index), lon_v(index), 50, dhw_v(index));
    ...
    title (['Plot of Degree Heating Weeks of North Pacific Stations in ',year_str],'FontSize',25);
    ...
    if yy == min(year_v) 
        imwrite(imind,cm,'npmap_dhw.gif','gif', 'Loopcount',inf); 
    
    else
        
        imwrite(imind,cm,'npmap_dhw.gif','gif','WriteMode','append');
