...
% 
% This script specifically plots the Maximum SST on a
% world map.
...
for aa = 1:length (hdf_names)
    ...
    max_sst = hdfread (myFile, 'CRW_SST_MAX');
    ...
    % under "variable_info". 
    % In this case, SST does not have large negative values,
    % and raw data needs to be divided by 100 to get data in °C.
    max_sst(max_sst==-7777)= NaN;
    max_sst = max_sst * 0.01;
    ...
    % each image frame
    title (['Map of Maximum Sea Surface Temperature in Year ',year_str],'FontSize',25);
    ...
    if aa == 1  
        ...
        imwrite(imind,cm,'sst_max.gif','gif', 'Loopcount',inf); 
    
    else
        % WriteMode and Append appends images to first image.
        imwrite(imind,cm,'sst_max.gif','gif','WriteMode','append');
