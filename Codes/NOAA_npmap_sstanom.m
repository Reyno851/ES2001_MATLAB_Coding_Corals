...
% 
% This script specifically plots data of SST anomalies over the time period of 2000
...
% empty vectors for latitude, longitude, year, and SST anomaly are created. These
...
% longitudes, sst anomaly and corresponding year of record
data_matrix = [lat_v lon_v year_v sstanom_v];
...
for yy = min(year_v) : 1 : max(year_v)
    ...
    % the corresponding latitude, longitude and sst anomaly data.
    ...
    c.Label.String = 'Sea Surface Temperature Anomaly (°C)';
    c.Label.FontSize = 12;
    
     % Use scatterm to plot all latitude and longitude of each NOAA Coral Reef Watch Virtual
    % Station and color of each station plot depends on the sst anomaly of the
    % station of each year.
    scatter_map = scatterm (lat_v(index), lon_v(index), 50, sstanom_v(index));
    ...
    title (['Plot of Sea Surface Temperature Anomaly in North Pacific Stations in ',year_str],'FontSize',25);
    ...
    if yy == min(year_v) 
        imwrite(imind,cm,'npmap_sstanom.gif','gif', 'Loopcount',inf); 
    
    else
        
        imwrite(imind,cm,'npmap_sstanom.gif','gif','WriteMode','append');
